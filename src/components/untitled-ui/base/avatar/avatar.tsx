...
export const Avatar = ({
  ...
  const renderMainContent = () => {
    ...
    if (initials) {
      return (
        <span className={cx("text-[#717680]", styles[size].initials)}>
          {initials}
        </span>
      );
    }
    ...
    if (PlaceholderIcon) {
      return (
        <PlaceholderIcon className={cx("text-[#a4a7ae]", styles[size].icon)} />
      );
    }

    return (
      placeholder || (
        <User01 className={cx("text-[#a4a7ae]", styles[size].icon)} />
      )
    );
  };
  ...
  return (
    <div
      data-avatar
      className={cx(
        "relative inline-flex shrink-0 items-center justify-center rounded-full bg-[#f5f5f5] outline-transparent",
        // Focus styles
        focusable &&
          "group-outline-focus-ring group-focus-visible:outline-2 group-focus-visible:outline-offset-2",
        contrastBorder && "outline outline-[#00000014]",
        styles[size].root,
        className,
      )}
    >
